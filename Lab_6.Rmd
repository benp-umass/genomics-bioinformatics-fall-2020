...
4. 
```{r}
daily_report_me <- read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/09-26-2020.csv")) %>% 
  rename(Long = "Long_") %>% 
...
state_join_me <- left_join(me_county, daily_report_me, by = c("subregion" = "Admin2")) 
# # plot state map
# ggplot(data = me_county, mapping = aes(x = long, y = lat, group = group)) + 
#   coord_fixed(1.3) + 
# # Add data layer
#   geom_polygon(data = state_join_me, aes(fill = Confirmed), color = "white") +
#     scale_fill_gradientn(colors = brewer.pal(n = 5, name = "BuGn"),
#                          trans = "log10") +
...
ggplotly(
  ...
  coord_fixed(1.3) + 
# Add data layer
  geom_polygon(data = state_join_me, aes(fill = Confirmed), color = "black") +
    scale_fill_gradientn(colors = brewer.pal(n = 5, name = "BuGn"),
                         trans = "log10") +
...
6. 
[Dashboard](lab_6_dashboard.html)